...
service / on new http:Listener(9090) {
    ...
    isolated resource function get patients() returns record {|anydata...;|}[]|sql:Error|error {
        stream<record {|anydata...;|}, sql:Error?> patients = check self.dbClient->/patients.get(targetType = db:Patient);
        return from var patient in patients
            select patient;
    }
